...
\begin{theorem} \label{th:matrix-form-n-to-n}
  \pt{$a(n,k)$, para $k \ge 0$ e $n > 0$, é dado por $\mathbf{1}_n^\intercal M_{n,k}^{n-1} \mathbf{1}_n$.}
  \en{$a(n,k)$, for $k \ge 0$ and $n > 0$, is given by $\mathbf{1}_n^\intercal M_{n,k}^{n-1} \mathbf{1}_n$.}
\end{theorem}
\begin{proof}
  \pt{Provaremos o fato mais forte de que o número de funções $k$-limitadas $f \colon [r] \to [n]$, $r \in [n]$, é dado por $\mathbf{1}_n^\intercal M_{n,k}^{r - 1} \mathbf{1}_n$, e que o número de tais funções com $f(r) = i$ é dado por $(M_{n,k}^{r - 1} \mathbf{1}_n)_i$. Procedemos por indução em $r$.}
  \en{We will prove the stronger fact that the number of $k$-limited functions $f \colon [r] \to [n]$, where $r \in [n]$, is given by $\mathbf{1}_n^\intercal M_{n,k}^{r - 1} \mathbf{1}_n$, and that the number of such functions with $f(r) = i$ is given by $(M_{n,k}^{r - 1} \mathbf{1}_n)_i$. We proceed by induction on $r$.}

  \pt{De fato, se $r = 1$, toda função $f \colon [r] \to [n]$ é constante, portanto há $n$ tais funções (uma para cada elemento de $[n]$), e $n = \mathbf{1}_n^\intercal I \mathbf{1}_n = \mathbf{1}_n^\intercal M_{n,k}^{1 - 1} \mathbf{1}_n$, sendo $I$ a matriz identidade $n \times n$, como queríamos.}
  \en{Indeed, if $r = 1$, every function $f \colon [r] \to [n]$ is constant, so there are $n$ such functions (one for each element of $[n]$), and $n = \mathbf{1}_n^\intercal I \mathbf{1}_n = \mathbf{1}_n^\intercal M_{n,k}^{1 - 1} \mathbf{1}_n$, where $I$ is the $n \times n$ identity matrix, as we wanted.}

  \pt{Se isso é válido para $r - 1$, então podemos encontrar o número de funções $f \colon [r] \to [n]$ contando o número de funções $k$-limitadas $g \colon [r-1] \to [n]$ das quais elas podem ser extensões. Se $f(r) = i$, então $f(r - 1) \in \{i - k, i - k + 1, \dots, i + k\} \cap [n]$, portanto a quantidade de tais funções $f$ é dada pela soma da quantidade de funções $k$-limitadas $g \colon [r-1] \to [n]$ com $g(r-1) \in \{i - k, i - k + 1, \dots, i + k\} \cap [n]$, que é $\sum_{j = 0}^{n-1} \chi_k(i, j) (M_{n,k}^{r-2} \mathbf{1}_n)_i$, que pela definição de multiplicação de matrizes é igual a $(M_{n,k} (M_{n,k}^{r-2} \mathbf{1}_n))_i = (M_{n,k}^{r-1} \mathbf{1}_n)_i$. Consequentemente, o número de todas as funções $k$-limitadas $f \colon [r] \to [n]$ é dado por $\mathbf{1}_n^\intercal M_{n,k}^{r-1} \mathbf{1}_n$, e o resultado segue para $n$, como queríamos.}
  \en{If this is valid for $r - 1$, then we can find the number of functions $f \colon [r] \to [n]$ by counting the number of $k$-limited functions $g \colon [r-1] \to [n]$ from which they can be extensions. If $f(r) = i$, then $f(r - 1) \in \{i - k, i - k + 1, \dots, i + k\} \cap [n]$, so the number of such functions $f$ is given by the sum of the number of $k$-limited functions $g \colon [r-1] \to [n]$ with $g(r-1) \in \{i - k, i - k + 1, \dots, i + k\} \cap [n]$, which is $\sum_{j = 0}^{n-1} \chi_k(i, j) (M_{n,k}^{r-2} \mathbf{1}_n)_i$, which by the definition of matrix multiplication equals $(M_{n,k} (M_{n,k}^{r-2} \mathbf{1}_n))_i = (M_{n,k}^{r-1} \mathbf{1}_n)_i$. Consequently, the number of all $k$-limited functions $f \colon [r] \to [n]$ is given by $\mathbf{1}_n^\intercal M_{n,k}^{r-1} \mathbf{1}_n$, and the result follows for $n$, as we wanted.}
\end{proof}

...
\begin{proof}
  ...
  \en{If this is valid for $r - 1$, then we can find the number of functions $f \colon [r] \to [n]$ by counting the number of $k$-limited functions $g \colon [r-1] \to [n]$ from which they can be extensions. If $f(r) = i$, then $f(r - 1) \in \{i - k, i - k + 1, \dots, i + k\} \cap [n]$, so the number of such functions $f$ is given by the sum of the number of $k$-limited functions $g \colon [r-1] \to [n]$ with $g(r-1) \in \{i - k, i - k + 1, \dots, i + k\} \cap [n]$, which is $\sum_{j = 0}^{n-1} \chi_k(i, j) (M_{n,k}^{r-1} \mathbf{1}_n)_i$, which by the definition of matrix multiplication equals $(M_{n,k} (M_{n,k}^{r-1} \mathbf{1}_n))_i = (M_{n,k}^r \mathbf{1}_n)_i$. Consequently, the number of all $k$-limited functions $f \colon [r] \to [n]$ is given by $\mathbf{1}_n^\intercal M_{n,k}^r \mathbf{1}_n$, and the result follows for $n$, as we wanted.}
\end{proof}

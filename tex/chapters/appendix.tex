...
\begin{definition*}
  \pt{Uma \textit{álgebra $C^\ast$} é uma álgebra de Banach complexa $\mathcal{A}$ munida de uma operação $^\ast : \mathcal{A} \to \mathcal{A}$, chamada de involução, tal que, para todos $a, b \in \mathcal{A}$ e $\lambda \in \mathbb{C}$, as seguintes propriedades são satisfeitas:}
  \en{A \textit{$C^\ast$-algebra} is a complex Banach algebra $\mathcal{A}$ equipped with an operation $^\ast : \mathcal{A} \to \mathcal{A}$, called involution, such that for all $a, b \in \mathcal{A}$ and $\lambda \in \mathbb{C}$, the following properties are satisfied:}
  \begin{enumerate}
...
\begin{definition*}
  ...
  \en{Let $H^2(\mathbb{T})$ be the Hardy space of functions on the unit circle $\mathbb{T}$. For a bounded function $a \in L^\infty(\mathbb{T})$, the \textit{Toeplitz operator} $T(a) : H^2(\mathbb{T}) \to H^2(\mathbb{T})$ is defined by $T(a)f=P(af)$, where $P : L^2(\mathbb{T}) \to H^2(\mathbb{T})$ is the Szegő projection operator, which projects a function in $L^2(\mathbb{T})$ onto the Hardy subspace $H^2(\mathbb{T})$.}
\end{definition*}
